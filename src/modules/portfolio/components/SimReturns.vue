...
<script>
    ...
    export default {
        ...
        setup() {
            onMounted(async () => {
                console.log('5 - SimReturns.onMounted -> await fetchAnalysisData')
            ...
            const fetchAnalysisData = async () => {
                ...
                let url = base_url + "/analysis/simreturns/" + id;
                console.log('5 - SimReturns.fetchAnalysisData')

                axios
                    .get(url, { headers: headers })
                    .then(function(response) {
                        console.log('SimReturns axios get')
    					console.log(url)
                        let data = JSON.parse(response.data);

                        // save the fetch data as it would be used when user is filtering data
                        store.commit(
                            "portfolio/updateSimReturnsGridData",
            ...
            const toggleChart = () => {
    			store.commit("portfolio/toggleSimReturnsGraph");
	    		showChart.value = store.getters["portfolio/getShowSimReturnsGraph"];
		    };
            ...
            const filterData = (isStarting) => {
                ...
                let storedData = JSON.parse(
                    JSON.stringify(store.getters["portfolio/getSimReturnsGridData"])
                );
